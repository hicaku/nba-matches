...
export default createStore({
	...
	actions: {
		async getLastWeekMatches({ commit }) {
			...
			commit(
				...
				response.data.data.sort(
					(match1: any, match2: any) => match1.id - match2.id
				)
			);
		},
		async getBoxScore({ commit, state }) {
			const url =
				"https://www.balldontlie.io/api/v1/stats?per_page=35&game_ids[]=" +
				state.matchId;
			const response = await axios.get(url);
		...
		async getPlayerDetails({ commit, state }, season) {
			...
			const url =
				"https://www.balldontlie.io/api/v1/stats?postseason=false&per_page=100&seasons[]=" +
				season +
				"&player_ids[]=" +
				state.playerId;
			const response = await axios.get(url);
			commit(
				...
				response.data.data
					.filter(
						(match) => match.min !== "0:00" && match.min !== "" && match.min
					)
					.sort((match1: any, match2: any) => match2.id - match1.id)
			);
			const urlPlayoffs =
				"https://www.balldontlie.io/api/v1/stats?postseason=true&per_page=100&seasons[]=" +
				season +
				"&player_ids[]=" +
				state.playerId;
			const responsePlayoffs = await axios.get(urlPlayoffs);
			commit(
				"setPlayerDetailsPlayoffs",
				responsePlayoffs.data.data
					.filter(
						(match) => match.min !== "0:00" && match.min !== "" && match.min
					)
					.sort((match1: any, match2: any) => match2.id - match1.id)

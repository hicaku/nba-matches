		...
		<div class="box-score-card">
			<div class="filteringFields">
				...
				<input
					...
					v-model="teamOnBoxScore"
				/>
				<label for="homeLabel">{{ matchDetail.home_team?.full_name }}</label>
				<input
					type="radio"
					name="teamSide"
					id="visitorLabel"
					...
					<tbody>
						<tr
							v-for="(player, index) in boxScoreFiltered.sort(
								(player1, player2) =>
									player2.min?.split(':').reduce((s, t) => s * 60 + +t, 0) -
									player1.min?.split(':').reduce((s, t) => s * 60 + +t, 0)
							)"
							:key="index"
						>
							...
							<td>{{ player.fg3m }}</td>
							<td>{{ player.fg3a }}</td>
							<td>{{ (player.fg3_pct * 100).toFixed(2) }}</td>
							<td>{{ player.ftm }}</td>
							<td>{{ player.fta }}</td>
							<td>{{ (player.ft_pct * 100).toFixed(2) }}</td>
						...
						<tr class="totals-row">
							...
							<th>
								{{
									boxScoreFiltered.reduce(function (sum, player) {
